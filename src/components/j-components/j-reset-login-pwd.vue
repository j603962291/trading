<template>
  <div class="login-block">
    ...
    <div class="title">
      {{$t('Safe.dialog.titles[0]')}}
    </div>
    <div class="subtitle">
      {{$t('Safe.dialog.subtitles[0]')}}
    </div>
    <div class="input-block">
      <label>{{$t('Safe.dialog.old_pwd')}}</label>
      <input  :type="current_pwd1" v-model="old_password" :placeholder="$t('Safe.dialog.placeholders[0]')" />
      <div class="input-tips">
        {{old_tip}}
      </div>
      <span class="see-icon" @click="handleCurrentPwd(1)" v-if="current_pwd1=='password'">
           <v-icon color="#999">mdi-eye-outline</v-icon>
        </span>
      <span class="see-icon" @click="handleCurrentPwd(1)" v-if="current_pwd1=='text'">
